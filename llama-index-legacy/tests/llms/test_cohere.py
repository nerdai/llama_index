...
def test_completion_model_basic(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(
        "llama_index.legacy.llms.cohere.completion_with_retry", mock_completion_with_retry
    )
...
async def test_async(monkeypatch: MonkeyPatch) -> None:
    mock_api_key = "fake_key"
    monkeypatch.setattr(
        "llama_index.legacy.llms.cohere.acompletion_with_retry", mock_acompletion_with_retry
    )
    llm = Cohere(model="command", api_key=mock_api_key)
    test_prompt = "test prompt"
